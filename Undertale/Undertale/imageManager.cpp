...
void imageManager::render(string strKey, HDC hdc, int destX, int destY, int width, int height)
{
	//이미지 찾아서 그냥 이미지클래스의 함수를 실행한다
	image* img = findImage(strKey);
...
void imageManager::scaleRender(string strKey, HDC hdc, int destX, int destY, int x, float scale)
{
	//이미지를 찾아서 그냥 이미지클래스의 함수를 실행시키면 됨
	image* img = findImage(strKey);
	if (img) img->scaleRender(hdc, destX, destY, x, scale);
}
